...
def save(num):
    ...
    c = con.cursor()
    get = c.execute("SELECT score FROM users where username = 'boss'")
    c.execute(f"update users set score = {num} where username = 'boss'")
    con.commit()

def get():
    con = sqlite3.connect('data.db')
    c = con.cursor()
    gett = c.execute("SELECT score FROM users where username = 'boss'")
    con.commit()
    return gett
def up(game):
    print("up")
